...
return _.template('<div style="\
   ...
   "><%=letter%></div>',args);}}
wn.get_cookie=function(c){var clist=(document.cookie+'').split(';');var cookies={};for(var i=0;i<clist.length;i++){var tmp=clist[i].split('=');cookies[$.trim(tmp[0])]=$.trim(tmp[1]);}
return cookies[c];}
...
return doc;},save:function(docstatus,callback){wn.call({method:'webnotes.model.doclist.save',args:{docs:this.doclist},callback:function(r){delete this.doclist;this.doclist=r.docs;if(this.doclist[0].get('name')!=this.name){this.rename();}}});},rename:function(){this.name=this.doclist[0].get('name');}});
/*
...
return[me.fieldselect.$select.find('option:selected').attr('table'),me.field.docfield.fieldname,me.$w.find('.condition').val(),val+''];}});wn.ui.FieldSelect=Class.extend({init:function(parent,doctype,filter_fields,with_blank){this.doctype=doctype;this.fields_by_name={};this.with_blank=with_blank;this.$select=$('<select>').appendTo(parent);if(filter_fields){for(var i in filter_fields)
...
this.page=page;$(this.page).fadeIn();$(this.page).trigger('show');this.page._route=window.location.hash;document.title=this.page.label;scroll(0,0);return this.page;}});wn.views.add_module_btn=function(parent,module){$(parent).append(_.template('<span class="label" style="margin-right: 8px; cursor: pointer;"\
...
wn.views.add_list_btn=function(parent,doctype){$(parent).append(_.template('<span class="label" style="margin-right: 8px; cursor: pointer;"\
 ...
 */
wn.provide('wn.views.pageview');wn.provide('wn.pages');wn.views.pageview={with_page:function(name,callback){if(!wn.model.has('Page',name)){wn.call({method:'webnotes.widgets.page.getpage',args:{'name':name},callback:callback});}else{callback();}},show:function(name){if(!name)
...
wn.views.Page=Class.extend({init:function(name,wrapper){this.name=name;var me=this;wn.pages[name]=this;if(name==window.page_name){this.wrapper=document.getElementById('page-'+name);this.wrapper.label=document.title||window.page_name;this.wrapper.page_name=window.page_name;wn.contents[window.page_name]=this.wrapper;}else{this.pagedoc=wn.model.get('Page',this.name);this.wrapper=wn.container.add_page(this.name);this.wrapper.label=this.pagedoc.get('title')||this.pagedoc.get('name');this.wrapper.page_name=this.pagedoc.get('name');this.wrapper.innerHTML=this.pagedoc.get('content');wn.dom.eval(this.pagedoc.get('script',''));wn.dom.set_style(this.pagedoc.get('style',''));this.trigger('load');}
...
return;}
if(r.server_messages)$.each(r.server_messages,function(i,v){msgprint(v);});if(r.exc)$.each(r.exc,function(i,v){console.log(v);});if(r['403']){wn.container.change_to('403');}}
